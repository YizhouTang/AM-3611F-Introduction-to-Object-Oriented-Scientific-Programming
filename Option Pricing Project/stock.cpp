...
#include <cassert>
	...
	double Stock::simulate_final(int N, double seed){
		double T =1.0;
		auto BM = this ->Brownian(seed,N);
		double* b = BM.first;
		double* W = BM.second;
...
public:
	...
	//Calculate the European call price thourgh the special discounted method
	double Discounted_Call_price(){
		int N = 1000;// Steps choose 1000
		double* Final_Price = new double[1000];
		double* probability = new double[1000];
		double sum =0.0;
		double sum1=0.0;
		//calculate average of the final prices
		double avg=0;
		double stdev = 0;
		
		for(int i = 0;i<N;i++){
			Final_Price[i]=simulate_final(N,i);
		}
		...
		//standard deviation
		for (int i = 0; i < N; i++){
			stdev += pow((Final_Price[i] - avg),2);
		}
		stdev = sqrt(stdev / N);
		//convert to standard normal distribution 
		for (int i = 0; i < N; i ++){
			probability[i] = (Final_Price[i] - avg)/stdev;
			probability[i] = norm_pdf(probability[i]);
			sum1+=probability[i];
		}
		
 		for (int i=0; i < N;i++){
			if (Final_Price[i]>K){// Situation the payoff that the call option exercise
				Final_Price[i] =Final_Price[i] - K;
			}
			else{ // Situation that not exercise
				Final_Price[i] = 0;
			}
			probability[i]=probability[i]/sum1;
			sum += Final_Price[i]*probability[i];	// Times their probability and add them up
		}
		sum = sum*exp(-r*T);// Discounted to the present value					
...
int main(int argc, const char * argv[])
{
	...
	Option option2(fileName,S0,K,r,T);
	double call_D = option2.Discounted_Call_price();
	std::cout<<"Call price(Discounted method): "<<call_D<<std::endl;
	
	double OptionPrice[100];
	double StockPrice[100];
	double OptionDelta[100];
	
	double stockprice_0 = 255;
	for(int i =0;i<100;i++){
	...
	for(int i =0; i<100;i++){
		writeoutput_Delta<<StockPrice[i]<<"   "<<OptionPrice[i]<<"\n";
	}
	...
	assert(writeoutput_Gamma.is_open());
		for(int i =0; i<100;i++){
		writeoutput_Gamma<<StockPrice[i]<<"   "<<OptionDelta[i]<<"\n";
	}
}
